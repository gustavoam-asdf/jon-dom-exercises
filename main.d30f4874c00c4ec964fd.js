...
var changeIcon = function (icon, state) {
    if (state) {
...
var clickOnPanel = function (evt) {
    var button = evt.target.closest("#" + elementIds.panel.button);
    if (button) {
        var menu = document.getElementById(elementIds.panel.menu.self);
        var icon = button.querySelector("." + elementIds.panel.button + "__icon");
        var isOpen = icon === null || icon === void 0 ? void 0 : icon.classList.contains(options.hamburger);
