...
var clockParts = function (localTime) {
    var usedTime = localTime;
    var currFormat = "";
    var hour12 = /(a|p)m/i.test(localTime);
    if (hour12) {
        var _a = localTime.split(/\s/i), time_1 = _a[0], format = _a[1];
        usedTime = time_1;
        currFormat = format;
    }
    var _b = usedTime.split(":"), hours = _b[0], minutes = _b[1], seconds = _b[2];
    var time = {
...
var ClockTemplate = function (_a) {
    var hours = _a.hours, minutes = _a.minutes, seconds = _a.seconds, separator = _a.separator, format = _a.format;
    var clock = document.createElement("div");
    clock.classList.add(Clock.className);
    clock.classList.add("hide");
    clock.innerHTML = "" + hours + separator + minutes + separator + seconds + (format ? format : "");
    return clock;
};
